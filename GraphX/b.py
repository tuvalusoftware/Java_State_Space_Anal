...
schema = sqlContext.read.json('./schema.json').select('id','P0','P1','P2').schema
...
# df = sqlContext.read.option('multiline', "true").json(sc.parallelize([ts]))
# df.show()
...
for id in raw['node']:
    ...
    for m in raw['node'][id]:
        px = eval(raw['node'][id][m])
        px_parse = []
        for k in px:
            px_parse.append(tuple(k))
        marking.append(px_parse)
